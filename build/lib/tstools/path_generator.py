    ...
    def get_optimal_distances(self):
        ...
        if len(formed_bonds) == 1:
            ...
            if atom_1.GetSymbol() == "H" or atom_2.GetSymbol() == "H":
                ...
                formed_bonds.update(self.broken_bonds)
        logger.info(formed_bonds)
        ...
        for i, bond in enumerate(formed_bonds):
            ...
            if possible_H_abstraction and i == 0:
                logger.info('estoy en lo primero del if')
                logger.info(f'{idx1} {idx2}')
                threshold_H = 0.75
                mol, mol_dict, smiles = self.get_mol_and_mol_dict_r(atom_i, atom_j, reactant_molecules, reactant_smiles)
            else:
                logger.info(i)
                logger.info('estoy en el else del if')
                logger.info(f'{idx1} {idx2}')
