...
def jointly_optimize_reactants_and_products(reactant_smiles, product_smiles):
    ...
    for i, conformer in enumerate(conformers):
        scanning_energies_dict = {}
        scanning_energies_list = []
        scanning_conformer_list = []
        for f in np.arange(0.5,2.5,0.1):
            new_conf = conformer.copy()
            new_conf.name = f'conf_{i}_constraint_{round(f,1)}'
            new_conf.constraints = Constraints()
            for key, val in breaking_constraints.items():
                new_conf.constraints.update({key: val * f})
            print(new_conf.constraints)

            new_conf.optimise(method = xtb)
            conf_energy = float(new_conf.energy - conformer.energy)
            scanning_energies_dict[conf_energy] = round(f,1)
            scanning_energies_list.append(conf_energy)
            scanning_conformer_list.append(new_conf)

        scanning_energies_list.reverse()
        for k, energy in enumerate(scanning_energies_list):
            if energy >= scanning_energies_list[k+1]:
                optimal_stretch = scanning_energies_dict[energy]
                optimal_conf = scanning_conformer_list[k]
                break
        
        new_conf = conformer.copy()

        with open('lol.out', 'w') as out:
            print(f'xtb conf_{i}_constraint_{optimal_stretch}_optimised_xtb.xyz --hess')
            subprocess.run(f'xtb conf_{i}_constraint_{optimal_stretch}_optimised_xtb.xyz --hess',
        ...
        raise KeyError
        # this is thrash!!!
        print(ade_mol, ade_mol.hessian)
        ade_mol.hessian(ade_hessian)

        print(ade_mol.normal_mode(6))

        
        
        # Compute Hessian and read in output to initialize autodE Hessian object
        # analyze normal mode
        # This only works for
...
def get_constraints(formed_bonds, broken_bonds, full_reactant_mol, full_reactant_dict, radii_dict):
    ...
    for bond in formed_bonds:
       ...
       formation_constraints[(mol_begin.GetIdx(), mol_end.GetIdx())] = \
        (radii_dict[mol_begin.GetAtomicNum()] + radii_dict[mol_end.GetAtomicNum()]) * 1.3 / 100
    ...
    for bond in broken_bonds:
        ...
        breaking_constraints[(atom_begin.GetIdx(), atom_end.GetIdx())] = \
                                (radii_dict[atom_begin.GetAtomicNum()] + radii_dict[atom_end.GetAtomicNum()]) * 1.3 / 100
