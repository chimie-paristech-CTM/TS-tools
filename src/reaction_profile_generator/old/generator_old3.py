...
def find_ts_guess(reactant_smiles, product_smiles, solvent=None, n_conf=5, fc_min=0.05, fc_max=0.9, fc_delta=0.10):
    ...
    formation_constraints_stretched.update((x, random.uniform(1.5, 2.5) * y) for x,y in formation_constraints_stretched.items() if x in formation_bonds_to_stretch)
    ...
    breaking_constraints_stretched.update((x, random.uniform(1.5, 2.5) * y) for x, y in breaking_constraints_stretched.items() if x in breaking_bonds_to_stretch)
    ...
    for conformer in conformers_to_do:
        ...
        #TODO: this is probably not the right order to do this!
        if preliminary_ts_guess_index is not None:
            path_xyz_files = get_path_xyz_files(atoms, coords, force_constant)
            run_hessian_calc(path_xyz_files[preliminary_ts_guess_index], charge)
            return path_xyz_files, preliminary_ts_guess_index
        else:
            print(potentials[-1])
